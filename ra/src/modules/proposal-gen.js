 ...
 * - Halaman 6: Bab III. Rincian Anggaran Biaya per Divisi Pekerjaan (Bagian 1)
 * - Halaman 7: Bab III. Rincian Anggaran Biaya per Divisi Pekerjaan (Bagian 2)
 * - Halaman 8: Bab IV. Jadwal Pelaksanaan & Diagram Kurva S
 * - Halaman 9: Bab V. Rekapitulasi Kebutuhan Sumber Daya
 * - Halaman 10: Bab VI & VII. Skema Pembayaran Termin & Lembar Pengesahan
...
window.ProposalGen = (function() {
  ...
  function generateProposalHtml(projParam = null, rabCalcParam = null, schedParam = null, resParam = null) {
    ...
    // =========================================================================
    // PARTISI DINAMIS BAB III & KALKULASI TOTAL HALAMAN CETAK FISIK
    // Mencegah Tumpahan Divisi / Overflow dan Memastikan Penomoran Halaman Presisi
    // =========================================================================
    ...
    // Total Halaman Fisik: Cover (1) + Kata Pengantar (1) + Daftar Isi (1) + Bab I (1) + Bab II (1) + Bab III (M) + Bab IV (1) + Bab V (1) + Bab VI/VII (1)
    const totalPages = 5 + bab3SheetCount + 3;

    const bab4PageNum = 5 + bab3SheetCount + 1;
    const bab5PageNum = 5 + bab3SheetCount + 2;
    const bab67PageNum = 5 + bab3SheetCount + 3;

    function renderRunningFooter(pageNum) {
      return `
        <div class="print-footer-block" style="margin-top: auto; border-top: none; padding-top: 4px; display: flex; justify-content: space-between; align-items: flex-end; font-size: 8pt; color: #475569;">
    ...
    const tocHtml = `
      <div class="proposal-page content-page" id="proposal-p3">
        ...
        <div class="toc-list" style="margin-top: 25px;">
          ...
          ${bab3Partitions.map((p, pIdx) => {
            const bab3P = 6 + pIdx;
            const partLbl = bab3SheetCount > 1 ? ` (BAGIAN ${pIdx + 1})` : '';
            return `<div class="toc-item"><span class="toc-title">BAB III. RINCIAN ANGGARAN BIAYA PER DIVISI${partLbl}</span><span class="toc-dots"></span><span class="toc-page">Halaman ${bab3P}</span></div>`;
          }).join('\n          ')}
          ...
          <div class="toc-item"><span class="toc-title">BAB VI &amp; VII. SKEMA PEMBAYARAN TERMIN &amp; LEMBAR PENGESAHAN</span><span class="toc-dots"></span><span class="toc-page">Halaman ${bab67PageNum}</span></div>
        </div>
    ...
    // =========================================================================
    // 6. Bab III: Rincian Detail RAB per Divisi (Partisi Dinamis Anti-Tumpahan)
    // =========================================================================
    ...
    const bab3PagesHtml = bab3Partitions.map((p, pIdx) => {
      ...
      const partLabel = bab3SheetCount > 1 ? ` (BAGIAN ${pIdx + 1}${pIdx === bab3SheetCount - 1 ? ' - SELESAI' : ''})` : '';
      return `
        <div class="proposal-page content-page" id="proposal-p${pageNum}">
          <div class="page-heading">
            <h2>BAB III. RINCIAN ANGGARAN BIAYA PEKERJAAN${partLabel}</h2>
            <div class="heading-line"></div>
          </div>
          ${buildDivisionsHtml(p.divs) || '<div class="text-muted p-3 text-center">Belum ada rincian divisi pekerjaan.</div>'}

          <!-- Running Footer Halaman ${pageNum} -->
          ${renderRunningFooter(pageNum)}
        </div>
      `;
    }).join('\n');
    // =========================================================================
    // 7. Bab IV: Kurva S & Jadwal Pelaksanaan (Halaman 8)
    // =========================================================================
    const bab4Html = `
      <div class="proposal-page content-page" id="proposal-p${bab4PageNum}">
        ...
        <!-- Running Footer Halaman ${bab4PageNum} -->
        ${renderRunningFooter(bab4PageNum)}
      </div>
    `;

    // =========================================================================
    // 8. Bab V: Rekapitulasi Kebutuhan Sumber Daya (Halaman 9)
    // =========================================================================
    let matRows = "";
    ...
    const bab5Html = `
      <div class="proposal-page content-page" id="proposal-p${bab5PageNum}">
        ...
        <!-- Running Footer Halaman ${bab5PageNum} -->
        ${renderRunningFooter(bab5PageNum)}
      </div>
    `;

    // =========================================================================
    // 9. Bab VI & VII: Skema Pembayaran & Lembar Pengesahan (Halaman 10)
    // =========================================================================
    ...
    const bab67Html = `
      <div class="proposal-page content-page" id="proposal-p${bab67PageNum}">
        <div class="page-heading">
        ...
        <!-- Running Footer Halaman ${bab67PageNum} -->
        ${renderRunningFooter(bab67PageNum)}
      </div>
    `;

    return coverHtml + introHtml + tocHtml + bab1Html + bab2Html + bab3PagesHtml + bab4Html + bab5Html + bab67Html;
  }
