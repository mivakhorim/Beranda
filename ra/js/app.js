...
window.App = (function() {
  ...
  function showFormModal(options) {
    ...
    document.getElementById("formModalSubmitBtn").textContent = submitText;
    document.getElementById("formModalCancelBtn").textContent = cancelText;

    if (options.customBodyHtml) {
      document.getElementById("formModalBody").innerHTML = options.customBodyHtml;
    } else {
      let fieldsHtml = "";
      fields.forEach(f => {
  ...
  function initModalEvents() {
    ...
    // Support keyboard Escape untuk menutup semua modal
  ...
  function setupGlobalEventListeners() {
    // Tombol Unduh PDF Langsung Saat Ini
    const downloadPdfBtn = document.getElementById("globalDownloadPdfBtn");
    if (downloadPdfBtn) {
      downloadPdfBtn.addEventListener("click", function() {
        const proj = window.ProjectManager.getActiveProject() || {};
        const filename = `${(proj.name || 'Proyek_RAB').replace(/\s+/g, '_')}_${currentTab.toUpperCase()}.pdf`;
        window.PrintEngine.downloadPdfDirect(`panel-${currentTab}`, filename);
      });
    }
  ...
  function downloadCurrentPagePdfDirect(panelId, defaultTitle = "Dokumen_RAB") {
    const proj = window.ProjectManager.getActiveProject() || {};
    const filename = `${defaultTitle}_${(proj.name || 'Proyek').replace(/\s+/g, '_')}.pdf`;
    window.PrintEngine.downloadPdfDirect(panelId, filename);
  }

  function downloadProposalPdfDirect() {
    const proj = window.ProjectManager.getActiveProject() || {};
    const filename = `Proposal_Rencana_Anggaran_Biaya_${(proj.name || 'Proyek').replace(/\s+/g, '_')}.pdf`;
    window.PrintEngine.downloadPdfDirect("proposalContent", filename, "portrait");
  }

  function downloadSingleBapPdfDirect(bapId) {
    const proj = window.ProjectManager.getActiveProject() || {};
    const filename = `BAP_${bapId}_${(proj.name || 'Proyek').replace(/\s+/g, '_')}.pdf`;
    window.PrintEngine.downloadPdfDirect(`bap-card-${bapId}`, filename, "portrait");
  }
      ...
      <div class="card">
        ...
        <div class="card-body p-0">
          ...
          <div class="d-mobile-only">
            ...
            <div class="rekap-mobile-grand-total-card">
              ...
              <div class="rekap-grand-total-terbilang">
                Terbilang: <strong>${window.CurrencyUtil.terbilangRupiah(rab.grandTotal)}</strong>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="rekap-mobile-card">
          <div class="rekap-mobile-card-header">
            <div class="d-flex align-items-center gap-1">
              ...
              <span class="badge badge-light" style="font-size: 10.5px;">${div.itemCount} Item</span>
            </div>
          ...
          <div>
            <div class="d-flex justify-content-between text-muted" style="font-size: 11px; margin-bottom: 3px;">
              <span>Bobot Pekerjaan:</span>
              <span class="font-bold text-dark">${window.CurrencyUtil.formatNumber(div.weightPercent, 2)}%</span>
            </div>
            <div class="rekap-mobile-progress-bar">
              <div class="rekap-mobile-progress-fill" style="width: ${Math.min(100, Math.max(0, div.weightPercent))}%;"></div>
            </div>
          </div>
      ...
      <div class="card">
        <div class="card-header">
          ...
          <div class="card-actions no-print d-flex align-items-center flex-wrap" style="gap: 8px;">
            ...
            <button class="btn btn-primary font-bold" onclick="window.PrintEngine.downloadPdfDirect('panel-rekap-rab', 'Rekapitulasi_RAB')">📥 Unduh PDF</button>
            <button class="btn btn-outline" onclick="window.PrintEngine.printDocument('panel-rekap-rab', 'Rekapitulasi_RAB')">🖨️ Cetak A4</button>
          </div>
        </div>
        <div class="card-body">
          <!-- Desktop Table View -->
  ...
  function renderDetailRabView() {
    ...
    proj.divisions.forEach(div => {
      ...
      (div.items || []).forEach((itm, idx) => {
        ...
        // 2. Mobile Responsive Touch Card (Zero Horizontal Scroll!)
        mobileCardsHtml += `
          <div class="rab-mobile-item-card" id="mob-item-${itm.id}">
            <div class="rab-mobile-card-top">
              <div class="d-flex align-items-center gap-1 flex-wrap">
                ...
                <span class="badge badge-light" style="font-size: 10px;">Item ${idx + 1}</span>
                <span class="badge badge-primary-subtle" style="font-size: 10px;">${itm.unit}</span>
              </div>
            ...
            <div class="rab-mobile-card-calc-row">
              ...
              <div class="text-right">
                <span class="calc-label">HSP:</span>
                <span class="calc-val font-bold"> ${window.CurrencyUtil.formatRupiah(itm.price, false, true)}</span>
              </div>
            </div>

            ${laborDetailHtml ? `<div class="mt-1" style="font-size: 11px;">${laborDetailHtml}</div>` : ''}
            ...
            <div class="rab-mobile-card-actions no-print">
              <button type="button" class="btn btn-sm btn-outline flex-1 d-flex align-items-center justify-content-center gap-1" onclick="App.openEditRabItemModal('${div.id}', '${itm.id}')">
                <span>✏️</span> <span>Ubah Item</span>
              </button>
              <button type="button" class="btn btn-sm btn-outline btn-danger d-flex align-items-center justify-content-center" style="min-width: 44px;" onclick="App.confirmDeleteRabItem('${div.id}', '${itm.id}')" title="Hapus Item">
                <span>🗑️</span>
              </button>
            </div>
          </div>
    ...
    container.innerHTML = `
      ...
      <div class="page-header no-print">
        ...
        <div class="page-header-actions d-flex align-items-center flex-wrap" style="gap: 8px;">
          ...
          <button class="btn btn-primary font-bold" onclick="App.downloadCurrentPagePdfDirect('panel-detail-rab', 'Rincian_Detail_RAB')">📥 Unduh PDF</button>
  ...
  function generateAhspRows(items, offset) {
    ...
    items.forEach((item, idx) => {
      ...
      let badgeHtml = "";
      if (isEdited) {
        badgeHtml = `<span class="badge badge-warning ml-2">Edited / Revisi</span>`;
      } else if (isCustom) {
      ...
      itemsHtml += `
        <tr>
          ...
          <td class="text-center no-print">
            ...
            ${isEdited ? `<button class="btn btn-sm btn-outline" title="Reset ke Standar PUPR" onclick="App.resetAhsp('${item.id}')">Reset</button>` : ''}
  ...
  function renderSumberDayaView() {
    ...
    res.materials.forEach((m, idx) => {
      ...
      matMobileCards += `
        <div class="resource-mobile-card">
          ...
          <div class="resource-mobile-card-row text-muted" style="font-size: 11px;">
            <span>Porsi Kelompok: <strong>${m.pctOfGroup ? m.pctOfGroup.toFixed(2) : '0.00'}%</strong></span>
            <span>Bobot Total: <strong>${m.pctOfTotal ? m.pctOfTotal.toFixed(2) : '0.00'}%</strong></span>
          </div>
        </div>
      `;
    });
    ...
    res.labor.forEach((l, idx) => {
      ...
      labMobileCards += `
        <div class="resource-mobile-card">
          ...
          <div class="resource-mobile-card-row">
            <span>Jumlah OH: <strong>${window.CurrencyUtil.formatNumber(l.qty, 2)} ${l.unit}</strong></span>
            <span>Upah/OH: <strong>${window.CurrencyUtil.formatRupiah(l.price, false, true)}</strong></span>
          </div>
          <div class="resource-mobile-card-row text-muted" style="font-size: 11px;">
            <span>Porsi Upah: <strong>${l.pctOfGroup ? l.pctOfGroup.toFixed(2) : '0.00'}%</strong></span>
            <span>Bobot Total: <strong>${l.pctOfTotal ? l.pctOfTotal.toFixed(2) : '0.00'}%</strong></span>
          </div>
        </div>
      `;
    });
    ...
    res.equipment.forEach((e, idx) => {
      ...
      eqMobileCards += `
        <div class="resource-mobile-card">
          ...
          <div class="resource-mobile-card-row text-muted" style="font-size: 11px;">
            <span>Porsi Alat: <strong>${e.pctOfGroup ? e.pctOfGroup.toFixed(2) : '0.00'}%</strong></span>
            <span>Bobot Total: <strong>${e.pctOfTotal ? e.pctOfTotal.toFixed(2) : '0.00'}%</strong></span>
          </div>
        </div>
      `;
    });
    container.innerHTML = `
      ...
      <div class="page-header no-print">
        ...
        <div class="page-header-actions d-flex align-items-center flex-wrap" style="gap: 8px;">
          <button class="btn btn-primary font-bold" onclick="window.PrintEngine.downloadPdfDirect('panel-sumberdaya', 'Rekapitulasi_Sumber_Daya')">📥 Unduh PDF</button>
          <button class="btn btn-outline" onclick="App.printCurrentPage('panel-sumberdaya', 'Rekap_Sumber_Daya')">🖨️ Cetak A4</button>
        </div>
      </div>
      ...
      <div class="mb-4 no-print" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(210px, 1fr)); gap: 14px;">
        ...
        <div style="background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 6px; padding: 12px 14px; box-shadow: 0 1px 2px rgba(0,0,0,0.03);">
          <span class="text-muted" style="font-size: 11px; text-transform: uppercase; font-weight: 700; color: #0f172a; display: block;">Grand Total Sumber Daya</span>
          <div style="font-size: 16px; font-weight: 800; color: #0f172a; margin-top: 3px;">${window.CurrencyUtil.formatRupiah(res.grandTotal, false, true)}</div>
          <span class="badge badge-light" style="font-size: 10.5px; margin-top: 5px; border: 1px solid #cbd5e1; color: #0f172a; font-weight: 700;">100.00% Total Anggaran</span>
        </div>
      </div>
    ...
    container.innerHTML = `
      ...
      <div class="page-header no-print">
        ...
        <div class="page-header-actions d-flex align-items-center flex-wrap" style="gap: 10px;">
          ...
          <button class="btn btn-primary font-bold" onclick="window.PrintEngine.downloadPdfDirect('panel-kurva-s', 'Kurva_S_Proyek', 'landscape')">📥 Unduh PDF</button>
          <button class="btn btn-outline" onclick="App.printCurrentPage('panel-kurva-s', 'Kurva_S_Proyek')">🖨️ Cetak A4</button>
        </div>
      </div>

      <!-- Tips Rotasi Layar Khusus Mobile -->
      <div class="d-mobile-only no-print mb-3" style="background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 8px; padding: 10px 12px; font-size: 12px; color: #1e40af; display: flex; align-items: center; gap: 8px;">
        <span style="font-size: 18px;">📱🔄</span>
        <div><strong>Tips Tampilan Ponsel:</strong> Putar ponsel ke orientasi <strong>Landscape (Mendatar)</strong> untuk membaca grafik Kurva S dan timeline jadwal mingguan dengan luas maksimal tanpa terpotong.</div>
      </div>
    ...
    container.innerHTML = `
      ...
      <div class="page-header no-print">
        ...
        <div class="page-header-actions d-flex align-items-center flex-wrap" style="gap: 8px;">
          ...
          <button class="btn btn-primary font-bold" onclick="window.PrintEngine.downloadPdfDirect('panel-kalender', 'Kalender_Proyek', 'landscape')">📥 Unduh PDF</button>
          <button class="btn btn-outline" onclick="App.printCurrentPage('panel-kalender', 'Kalender_Proyek')">🖨️ Cetak A4</button>
        </div>
      </div>
        ...
        <div class="card mb-4 bap-card-item" id="bap-card-${bap.id || bap.bapNumber}">
          <div class="card-header no-print d-flex justify-content-between align-items-center flex-wrap" style="background-color: #f8fafc; border-bottom: 1px solid var(--color-border); padding: 12px 18px; gap: 10px;">
            ...
            <div class="card-actions d-flex align-items-center flex-wrap" style="gap: 8px;">
              ...
              <button class="btn btn-sm btn-primary font-bold" onclick="App.downloadSingleBapPdfDirect('${bap.id || bap.bapNumber}')" style="display: inline-flex; align-items: center; gap: 5px;">
                📥 Unduh PDF BAP
              </button>
  ...
  function renderProposalView() {
    ...
    container.innerHTML = `
      <div class="page-header no-print">
        ...
        <div class="page-header-actions d-flex align-items-center flex-wrap" style="gap: 8px;">
          <button class="btn btn-primary font-bold" onclick="App.downloadProposalPdfDirect()">📥 Unduh File PDF Langsung</button>
          <button class="btn btn-outline" onclick="App.printProposal()">🖨️ Cetak / Preview A4</button>
        </div>
      </div>
  ...
  function resetAhsp(ahspId) {
    showConfirmModal({
      title: "Reset Analisis AHSP",
      message: "Apakah Anda yakin ingin mengembalikan koefisien dan harga analisis ini ke standar baku SE Bina Konstruksi No. 47/SE/Dk/2026?",
      confirmText: "Ya, Reset ke Standar",
      onConfirm: () => {
        window.AhspEngine.resetToStandard(ahspId);
        renderCurrentTabContent();
      }
    });
  }
  ...
  function openCreateProjectModal() {
    showFormModal({
      ...
      fields: [
        ...
        {
          name: "templateMode",
          label: "Inisialisasi Data Proyek (Pilihan Template)",
          type: "select",
          value: "template",
          options: [
            { value: "template", label: "Template Standar Lengkap (10 Divisi, Item AHSP, Kalender & S-Curve Siap Pakai - Rekomendasi)" },
            { value: "blank", label: "Kerangka Kosong (Mulai dari Nol)" }
          ],
          help: "Pilih template standar agar proyek langsung terisi item pekerjaan lengkap, kalender 1 tahun, dan Kurva S"
        }
      ],
      submitText: "Buat Proyek",
      onSubmit: (data) => {
        const useTemplate = data.templateMode !== "blank";
        const newProj = window.ProjectManager.createProject(data.name, data.owner, data.location, "std", useTemplate);
        if (newProj) {
  ...
  function handleOverheadChange(ahspId, newOverhead) {
    const ahsp = window.AhspEngine.getAhspById(ahspId);
    if (ahsp) {
      ahsp.overhead_percent = parseFloat(newOverhead) || 15;
      window.AhspEngine.saveEditedAhsp(ahsp);
      openAhspDetailModal(ahspId);
      renderCurrentTabContent();
    }
  ...
  function printSelectedAhspData() {
    ...
    if (usedAhspList.length === 0) {
      ...
      usedAhspList.forEach((item, aIdx) => {
        ...
        const overheadPercent = Number(item.overhead_percent !== undefined ? item.overhead_percent : 15);
      ...
      body.innerHTML = `
        <div class="mb-3">
          <div style="display: flex; gap: 10px; margin-bottom: 8px;">
            ...
            <span class="badge badge-warning">Overhead & Profit: ${ahsp.overhead_percent || 15}%</span>
